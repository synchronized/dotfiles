f() {
    ...
    echo "$_f_options"
    _f_options="$@"
    if [ -z "${_f_options}" ]; then
        _f_options=" "
    else
        _f_options=" $_f_options "
    fi
    ...
    echo $program$_f_options$arguments >> ~/.bash_history
...
fshow() {
  ...
  git log --graph --color=always \
      --pretty=format:'%Cred%h%Creset -%C(yellow)%d%Creset %s %Cgreen(%cr) %Cblue<%an>%Creset' --abbrev-commit --date=relative --all |
